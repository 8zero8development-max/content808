            ...
            <div className="border border-[hsl(var(--th-border))] rounded-xl overflow-hidden animate-fadeIn">
                <div className="grid grid-cols-7 bg-[hsl(var(--th-surface))]">
                    {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
                        <div key={d} className="px-2 py-2.5 text-xs font-medium text-[hsl(var(--th-text-muted))] text-center border-b border-[hsl(var(--th-border))]">{d}</div>
                    ))}
                </div>
                {Array.from({ length: 5 }).map((_, wi) => (
                    <div key={wi} className="grid grid-cols-7">
                        {Array.from({ length: 7 }).map((_, di) => (
                            <div key={di} className="min-h-[100px] border-b border-r border-[hsl(var(--th-border)/0.4)] p-2">
        ...
        return (
            <div className="grid grid-cols-7 gap-3 animate-fadeIn">
                {Array.from({ length: 7 }).map((_, i) => (
                    <div key={i} className="rounded-xl border border-[hsl(var(--th-border))] min-h-[200px]">
                        <div className="px-3 py-3 border-b border-[hsl(var(--th-border)/0.4)]">
        ...
        return (
            <div className="animate-fadeIn space-y-0">
                {Array.from({ length: 12 }).map((_, i) => (
                    <div key={i} className="flex border-b border-[hsl(var(--th-border)/0.4)] min-h-[48px]">
